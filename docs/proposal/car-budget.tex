...
\begin{itemize}
  ...
  and a motorless Tamiya 1:10 touring chassis. The camera goes on this car. The M1 is a
  full-size-vehicle sensor (about 0.7~kg, half-metre blind zone) and does not belong on a
  1:10 car; it waits for a larger platform. The Tamiya has no motor, controller or sensor
  deck, does not match the simulator's car, and would cost a semester of mechanical work to
  save about \$300; it stays a bench mule.
...
Item & USD & \\
\midrule
Cardboard walls: flattened boxes, 20~cm strips, taped in loops & 0 & recommended to start \\
Floor tape (start line, lane marks) & 15 & \\
FSAE-style cones, 3D-printed at 1:10, 100 pieces & 15 & about 10~g of filament each \\
Foam pipe insulation or pool noodles as bumpers on corners & 15 & optional \\
Flexible aluminium duct, 33~cm, 30~m (the league's walls) & 130 & later, if the track gets daily use \\
\bottomrule
\end{tabularx}

\smallskip
\textbf{Cardboard is the right start.} The lidar sees cardboard as well as duct; a 20~cm wall
is well above the scan plane; a wall that folds when hit is kinder to the car than one that
does not; and the cost is zero. Its faults are real: it gets flattened, it has to be re-taped
every session, and it does not store. Duct is what to buy once the team is running the track
several times a week and is tired of rebuilding it. The cones are cheap and worth printing on
day one, because the cone track is where the camera work happens.
...
\begin{itemize}
  ...
  \item \textbf{Cardboard walls have a life of weeks.} Budget an hour per session for the
  track, and plan on duct in the second half of the semester.
  \item \textbf{Out of scope for this budget.} The M1 lidar and any 1:5-scale vehicle, the
  Alameda track, a second car, and physical competitions. All are possible later; none are
  needed to get value from this car.
\end{itemize}
