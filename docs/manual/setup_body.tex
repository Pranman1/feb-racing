...
\texttt{starter\_kit/}, copy it the same way as the first one. The cone track has three: the
baseline cone driver, the cone racer (mapping lap, raceline, MPC), which needs SciPy and
CasADi installed once into \texttt{stack/.pydeps}, and the team's own cone ordering (C++,
built by the devkit on start), which the racer uses when it is there:
...
starter_kit/feb_cone_racer/install_deps.sh     # once, with the devkit running
./feb-sim run --track loop_cones --look visual --stack "ros2 launch feb_cone_racer race.launch.py"
\end{lstlisting}

The drivers need the Visual look: the camera's colour thresholds are tuned for the dressed
scene. \texttt{docs/starter-kit.md} describes what each package does.
