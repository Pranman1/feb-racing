...
\texttt{starter\_kit/}, copy it the same way as the first one. The cone track has two: the
baseline cone driver and the cone racer (mapping lap, raceline, MPC), which needs SciPy and
CasADi installed once into \texttt{stack/.pydeps}:
...
Both need the Visual look: the camera's colour thresholds are tuned for the dressed scene.
\texttt{docs/starter-kit.md} describes what each package does.
